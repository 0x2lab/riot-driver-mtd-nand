...
inline size_t nand_onfi_write_cycle(const nand_onfi_t* const nand, const uint16_t* const cycle_data, const uint32_t cycle_write_enable_post_delay_ns, const uint32_t cycle_write_disable_post_delay_ns) {
    return nand_onfi_write_io(nand, cycle_data, cycle_write_enable_post_delay_ns, cycle_write_disable_post_delay_ns);
}

inline size_t nand_onfi_write_raw(const nand_onfi_t* const nand, const uint16_t* const data, const size_t data_size, const uint32_t cycle_write_enable_post_delay_ns, const uint32_t cycle_write_disable_post_delay_ns) {
    size_t ret_size = 0;
...
inline size_t nand_onfi_write_cmd(const nand_onfi_t* const nand, const uint8_t* const cmd, const uint32_t cycle_write_enable_post_delay_ns, const uint32_t cycle_write_disable_post_delay_ns) {
...
inline size_t nand_onfi_write_addr(const nand_onfi_t* const nand, const uint64_t addr[], const uint32_t cycle_write_enable_post_delay_ns, const uint32_t cycle_write_disable_post_delay_ns) {
    size_t ret_size = 0;
...
inline size_t nand_onfi_write_addr_column_single(const nand_onfi_t* const nand, const uint16_t* const addr_column_single_cycle_data, const uint32_t cycle_write_enable_post_delay_ns, const uint32_t cycle_write_disable_post_delay_ns) {
...
inline size_t nand_onfi_write_addr_row_single(const nand_onfi_t* const nand, const uint16_t* const addr_row_single_cycle_data, const uint32_t cycle_write_enable_post_delay_ns, const uint32_t cycle_write_disable_post_delay_ns) {
...
inline size_t nand_onfi_read_cycle(const nand_onfi_t* const nand, uint16_t* const out_cycle_data, const uint32_t cycle_read_enable_post_delay_ns, const uint32_t cycle_read_disable_post_delay_ns) {
    return nand_onfi_read_io(nand, out_cycle_data, cycle_read_enable_post_delay_ns, cycle_read_disable_post_delay_ns);
}

inline size_t nand_onfi_read_raw(const nand_onfi_t* const nand, uint16_t* const out_buffer, const size_t buffer_size, const uint32_t cycle_read_enable_post_delay_ns, const uint32_t cycle_read_disable_post_delay_ns) {
    size_t ret_size = 0;
...
inline void nand_onfi_set_pin_default(const nand_onfi_t* const nand) {
    nand_onfi_set_ctrl_pin(nand);
    nand_onfi_set_io_pin_write(nand);
}

inline void nand_onfi_set_latch_command(const nand_onfi_t* const nand) {
    gpio_write(nand->params.ale, 0);
    gpio_write(nand->params.cle, 1);
}

inline void nand_onfi_set_latch_address(const nand_onfi_t* const nand) {
    gpio_write(nand->params.cle, 0);
    gpio_write(nand->params.ale, 1);
}

inline void nand_onfi_set_latch_raw(const nand_onfi_t* const nand) {
    gpio_write(nand->params.cle, 0);
    gpio_write(nand->params.ale, 0);
}

inline void nand_onfi_set_read_enable(const nand_onfi_t* const nand) {
    gpio_write(nand->params.re, 0);
}

inline void nand_onfi_set_read_disable(const nand_onfi_t* const nand) {
    gpio_write(nand->params.re, 1);
}

inline void nand_onfi_set_write_enable(const nand_onfi_t* const nand) {
    gpio_write(nand->params.we, 0);
}

inline void nand_onfi_set_write_disable(const nand_onfi_t* const nand) {
    gpio_write(nand->params.we, 1);
}

inline void nand_onfi_set_write_protect_enable(const nand_onfi_t* const nand) {
    gpio_write(nand->params.wp, 0);
}

inline void nand_onfi_set_write_protect_disable(const nand_onfi_t* const nand) {
    gpio_write(nand->params.wp, 1);
}

inline void nand_onfi_set_chip_enable(const nand_onfi_t* const nand, const uint8_t lun_no) {
    switch(lun_no) {
    case 0:
...
inline void nand_onfi_set_chip_disable(const nand_onfi_t* const nand, const uint8_t lun_no) {
    switch(lun_no) {
    case 0:
...
inline void nand_onfi_wait(const uint32_t delay_ns) {
    if(delay_ns != 0) {
        ztimer_sleep(ZTIMER_USEC, delay_ns / NAND_ONFI_TIMING_MICROSEC(1));
    }
}

inline size_t nand_onfi_all_pages_count(const nand_onfi_t* const nand) {
    return nand->pages_per_block * nand->blocks_per_lun * nand->lun_count;
}

inline size_t nand_onfi_one_page_size(const nand_onfi_t* const nand) {
    return nand->data_bytes_per_page + nand->spare_bytes_per_page;
}

inline size_t nand_onfi_all_data_bytes_size(const nand_onfi_t* const nand) {
    return nand->data_bytes_per_page * nand_onfi_all_pages_count(nand);
}

inline size_t nand_onfi_all_spare_bytes_size(const nand_onfi_t* const nand) {
    return nand->spare_bytes_per_page * nand_onfi_all_pages_count(nand);
}

inline size_t nand_onfi_all_pages_size(const nand_onfi_t* const nand) {
    return nand_onfi_all_data_bytes_size(nand) + nand_onfi_all_spare_bytes_size(nand);
}

inline uint64_t nand_onfi_offset_to_addr_column(const uint64_t offset) {
    return offset;
}

inline uint64_t nand_onfi_page_num_to_addr_row(const uint64_t page_num) {
    return page_num;
}

inline uint64_t nand_onfi_addr_flat_to_addr_column(const nand_onfi_t* const nand, const uint64_t addr_flat) {
    return addr_flat % nand_onfi_one_page_size(nand);
}

inline uint64_t nand_onfi_addr_flat_to_addr_row(const nand_onfi_t* const nand, const uint64_t addr_flat) {
    return addr_flat / nand_onfi_one_page_size(nand);
}

inline uint64_t nand_onfi_addr_to_addr_flat(const nand_onfi_t* const nand, const uint64_t addr_row, const uint64_t addr_column) {
