...
size_t nand_onfi_read_parameter_page(nand_onfi_t* const nand_onfi, const uint8_t this_lun_no, const nand_cmd_t* const pp_cmd, uint16_t* const bytes_pp, const size_t bytes_pp_max_size) {
    ...
    {
        nand_raw_t _raw_store = {
            .raw_size = bytes_pp_size,
            .buffer = bytes_pp,
            .buffer_size = bytes_pp_size,
            .current_buffer_seq = 0,
            .current_raw_offset = 0
        };
        memcpy(raw_store, &_raw_store, sizeof(_raw_store));
    ...
    nand_onfi_run_cmd(nand_onfi, pp_cmd, cmd_params, err);
    ...
    const bool   is_DDR      = nand_check_DDR(bytes_pp, bytes_pp_size);
    const size_t folded_size = is_DDR ? nand_fold_DDR_repeat_bytes(bytes_pp, bytes_pp_size, 0x00) : bytes_pp_size;

    for(size_t pos = folded_size; pos < folded_size; ++pos) {
        bytes_pp[pos] = 0x00;
    }
