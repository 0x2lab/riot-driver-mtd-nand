...
size_t nand_onfi_read_raw(const nand_onfi_t* const nand, uint16_t* const out_buffer, const size_t buffer_size, const uint32_t cycle_read_enable_post_delay_ns, const uint32_t cycle_read_disable_post_delay_ns) {
    ...
    for(size_t seq = 0; seq < buffer_size; ++seq) {
        print_str("TESTR\r\n");
        print_u32_hex((uint32_t)(uintptr_t)out_buffer);
        print_str("\r\n");
        print_u32_dec((uint32_t)buffer_size);
        print_str("\r\n");
        print_u32_hex((uint32_t)(uintptr_t)&(out_buffer[seq]));
        print_str("\r\n");
        print_u32_hex((uint32_t)out_buffer[seq]);
        print_str("\r\n");
        ret_size += nand_onfi_read_io(nand, &(out_buffer[seq]), cycle_read_enable_post_delay_ns, cycle_read_disable_post_delay_ns);
        print_str("TESTS\r\n");
    }
    print_str("TESTT\r\n");
